...
namespace OM3D
{
    ...
    void Program::set_uniform(u32 name_hash, bool value)
    {
        if (const int loc = find_location(name_hash); loc >= 0)
        {
            glProgramUniform1f(_handle.get(), loc, value);
    ...
    {
        const auto loc =
            glGetUniformLocation(this->_handle.get(), name.c_str());
        if (loc >= 0)
        {
            glProgramUniformMatrix4fv(_handle.get(), loc, count, false,
                                      glm::value_ptr(*value));
        }
    }
} // namespace OM3D
