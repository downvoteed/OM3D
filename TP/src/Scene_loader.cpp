...
namespace OM3D {
    ...
    static bool decode_attrib_buffer(const tinygltf::Model& gltf, const std::string& name, tinygltf::Accessor& accessor,
    ...
    static Result<TextureData> build_texture_data(const tinygltf::Image& image, bool as_sRGB) {
        if (image.bits != 8 && image.pixel_type != TINYGLTF_COMPONENT_TYPE_BYTE && image.pixel_type !=
    ...
    Result<std::unique_ptr<Scene>> Scene::from_gltf(const std::string& file_name) {
        ...
        for (auto [node_index, light_index]: light_nodes) {
            ...
            auto scene_object = SceneObject(std::make_shared<StaticMesh>(sphereMesh.value),
            ...
            const glm::vec3 color = glm::vec3(float(gltf_light.color[0]), float(gltf_light.color[1]),
                                              float(gltf_light.color[2])) * float(gltf_light.intensity);;
